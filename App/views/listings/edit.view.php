...
<section class="flex justify-center items-center mt-20">
    <div class="bg-white p-8 rounded-lg shadow-md w-full md:w-600 mx-6">
        ...
        <form method="POST" action="/listings/<?= $listing->id ?>">
            ...
            <div class="mb-4">
                <input type="text" name="company" placeholder="公司名称" class="w-full px-4 py-2 border rounded focus:outline-none" value="<?= $listing->company ?? '' ?>" />
            </div>
            <div class="mb-4">
                <input type="text" name="address" placeholder="地址" class="w-full px-4 py-2 border rounded focus:outline-none" value="<?= $listing->address ?? '' ?>" />
            </div>
            <div class="mb-4">
                <input type="text" name="city" placeholder="城市" class="w-full px-4 py-2 border rounded focus:outline-none" value="<?= $listing->city ?? '' ?>" />
            </div>
            <div class="mb-4">
                <input type="text" name="province" placeholder="省份" class="w-full px-4 py-2 border rounded focus:outline-none" value="<?= $listing->province ?? '' ?>" />
            </div>
            <div class="mb-4">
                <input type="text" name="phone" placeholder="电话" class="w-full px-4 py-2 border rounded focus:outline-none" value="<?= $listing->phone ?? '' ?>" />
            </div>
            <div class="mb-4">
                <input type="email" name="email" placeholder="接受申请的电子邮箱地址" class="w-full px-4 py-2 border rounded focus:outline-none" value="<?= $listing->email ?? '' ?>" />
            </div>
